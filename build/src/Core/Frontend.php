...
class Frontend
{
    ...
    private function printColumn(array $column_data): void
    {
        ...
        if (!empty($column_data['elements'])) {
            foreach ($column_data['elements'] as $widget_data) {
                ...
                } elseif ('tabs' === $widget_data['elType']) {
                    $this->printTabs($widget_data);
                } else {
    ...
    private function printTabs(array $tabs_data): void
    {
        $tabs_obj = Plugin::instance()->elementsManager->getElement('tabs');

        if (false === $tabs_obj) {
            return;
        }

        if (empty($tabs_data['settings'])) {
            $tabs_data['settings'] = [];
        }

        $instance = $tabs_obj->getParseValues($tabs_data['settings']);
        $tab_children = !empty($tabs_data['elements']) ? $tabs_data['elements'] : [];

        $tabs_obj->beforeRender($instance, $tabs_data['id'], $tabs_data);

        $tabs_obj->renderNav($tab_children, $tabs_data['id']);

        foreach ($tab_children as $index => $tab_data) {
            if (!isset($tab_data['elType']) || 'tab' !== $tab_data['elType']) {
                continue;
            }
            $this->printTab($tab_data, $tabs_data['id'], (int) $index);
        }

        $tabs_obj->afterRenderContent();
        $tabs_obj->afterRender($instance, $tabs_data['id'], $tabs_data);
    }

    private function printTab(array $tab_data, string $tabs_id, int $index): void
    {
        ...
        if (empty($tab_data['settings'])) {
            $tab_data['settings'] = [];
        }

        $instance = $tab_obj->getParseValues($tab_data['settings']);

        $render_data = $tab_data;
        $render_data['_tabs_id'] = $tabs_id;
        $render_data['_index'] = $index;

        $tab_obj->beforeRender($instance, $tab_data['id'], $render_data);

        if (!empty($tab_data['elements'])) {
            foreach ($tab_data['elements'] as $child) {
                if (!isset($child['elType'])) {
                    continue;
                }
                if ('section' === $child['elType']) {
                    $this->printSection($child);
                } elseif ('tabs' === $child['elType']) {
                    $this->printTabs($child);
                } else {
                    $this->printWidget($child);
                }
            }
        }

        $tab_obj->afterRender($instance, $tab_data['id'], $tab_data);
    }
