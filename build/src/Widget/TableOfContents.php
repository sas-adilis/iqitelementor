...
class TableOfContents extends WidgetBase
{
    ...
    */
    ...
    {
        $rawHeadings = $optionsSource['headings_by_tags'] ?? ['h2' => 'h2'];
        $headingsByTags = [];
        if (is_string($rawHeadings)) {
            $headingsByTags = array_values(array_filter(array_map('trim', explode(',', $rawHeadings))));
        } elseif (is_array($rawHeadings)) {
            foreach ($rawHeadings as $key => $value) {
                if (is_int($key)) {
                    if ($value !== '' && $value !== null) {
                        $headingsByTags[] = $value;
                    }
