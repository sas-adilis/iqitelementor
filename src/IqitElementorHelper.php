...
class IqitElementorHelper
{
    ...
    public static function getElementorWidgetInstanceByName($name)
    {
        if (isset($WIDGETS_INSTANCES[$name])) {
            return $WIDGETS_INSTANCES[$name];
        }

        $className = 'Elementor\Widget_' . ucfirst($name);
        if (!class_exists($className)) {
            $className = '\Widget_' . ucfirst($name);
            if (!class_exists($className)) {
                return self::$WIDGETS_INSTANCES[$name] = null;
            }
        }

        self::$WIDGETS_INSTANCES[$name] = new $className(Context::getContext());
    ...
    public static function stringToArrayOfColors(string $string)
    {
        ...
        return array_filter($colors, function ($color) {
            return Validate::isColor($color);
        });
    }

    public static function arrayToStringWithComma(array $array): string
    {
        return implode(',', $array);
    }

    public static function cleanStringOfColors(string $string): string
    {
        return self::arrayToStringWithComma(self::stringToArrayOfColors($string));
    }
}
