var ControlBaseItemView = require( 'elementor-views/controls/base' ),
	ControlAutocompleteItemsItemView;

ControlAutocompleteItemsItemView = ControlBaseItemView.extend( {
	...
	onShow: function () {
		...
		$(this.ui.searchInput).autocomplete(self.getSearchUrl(), p_auto_settings).result(function (event, data, formatted) {
			if (data == null)
				return false;
	...
	getSearchUrl: function() {
		return $(this.ui.searchInput).attr('data-search-url') || '';
	},

	getRetrieveUrl: function() {
		return $(this.ui.searchInput).attr('data-get-url') || '';
	},
	...
	insertPosts: function(ids) {
		...
		var posts = null;
		var self = this;
		var getUrl = this.getRetrieveUrl();
		...
		$.ajax( {
			...
			success: function(response) {
				...
				_.each( data, function( item ) {
					$(self.ui.selectedPreview).append('<div class="elementor-post" data-post-id="' + item.id + '"><div class="elementor-repeater-row-handle-sortable"><i class="fa fa-ellipsis-v"></i></div><img class="elementor-post-image" src="' + item.image + '" />' +
						'<div class="elementor-post-info"><span class="elementor-post-reference">(id: ' + item.id + ')</span>'
						+ item.name
						+ '<button data-post-id="' + item.id + '" class="elementor-post-remove"><i class="fa fa-remove"></i></button></div></div>');
				});
			}
		} );
		return posts;
...
module.exports = ControlAutocompleteItemsItemView;
